...
public class PostResource extends Resource {
	...
	public static String getPost(String post_id) {
		String post_json = Redis.LRUDictionaryGet(Redis.TOP_POSTS, post_id);
		if(post_json == null) {
			Post post = CosmosClient.getByIdUnparse(POSTS_CONTAINER, post_id, Post.class);
		...
		if(total_likes == null) {
			if(Redis.ACTIVE) {			
				...
				if(it.hasNext() ) {
					...
					Redis.LRUHyperLogPut(Redis.TOTAL_LIKES, Redis.TOTAL_LIKES_LIMIT, post_id, likes.stream().map(l -> l.getId()).collect(Collectors.toList()));
	...
	public static Entry<String,List<PostWithReplies>> getReplies(String post_id, String continuation_token, int page_size) {
		...
		if(pair == null) {
			...
			entry = CosmosClient.queryAndUnparsePaginated(PostResource.POSTS_CONTAINER, query_replies, unparsed_continuation_token, page_size, PostWithReplies.class);
			...
			if(first) {
				first = false;
				Redis.LRUHyperLogPut(Redis.TOTAL_REPLIES, Redis.TOTAL_REPLIES_LIMIT, post_id, replies_ids);
			} else {
				Redis.LRUHyperLogUpdate(Redis.TOTAL_REPLIES, post_id, replies_ids, false);
			}
		} else {
			replies = GSON.fromJson(pair.getKey(), new TypeToken<List<PostWithReplies>>(){}.getType());
	...
	public static long getTotalReplies(String post_id, PostWithReplies post, int page_size) {
		Long total_replies = Redis.LRUHyperLogGet(Redis.TOTAL_REPLIES, post_id);
	...
	public static long getRecentLikes(String post_id, long time_s) {
		Long n_likes = Redis.LRUHyperLogGet(Redis.DAYLY_LIKES, post_id);
		if(n_likes == null) {
			if(Redis.ACTIVE) {
				...
				if(it.hasNext() ) {
					List<Like> likes = it.next().getResults().stream().map((Document d) -> GSON.fromJson(d.toJson(), Like.class)).collect(Collectors.toList());
					Redis.LRUHyperLogPut(Redis.DAYLY_LIKES, Redis.DAYLY_LIKES_LIMIT, post_id, likes.stream().map(l -> l.getId()).collect(Collectors.toList()));
				...
				while( it.hasNext() ) {
					List<String> likes = it.next().getResults().stream().map((Document d) -> GSON.fromJson(d.toJson(), Like.class).getId()).collect(Collectors.toList());
					Redis.LRUHyperLogUpdate(Redis.DAYLY_LIKES, post_id, likes, false);
	...
	public static long getRecentReplies(String post_id, long time_s) {
		Long n_replies = Redis.LRUHyperLogGet(Redis.DAYLY_REPLIES, post_id);
		if(n_replies == null) {
			if(Redis.ACTIVE) {
				...
				if(it.hasNext() ) {
					...
					Redis.LRUHyperLogPut(Redis.DAYLY_REPLIES, Redis.DAYLY_REPLIES_LIMIT, post_id, replies.stream().map(r -> r.getId()).collect(Collectors.toList()));
				...
				while( it.hasNext() ) {
					...
					Redis.LRUHyperLogUpdate(Redis.DAYLY_REPLIES, post_id, posts, false);
